 ...
 *
 * Google and SUSE (Okta) share the generic OIDC path: discovery document
...
export async function fetchVerifiedEmail(provider, env, { code, redirectUri, pkceVerifier, nonce }) {
  ...
  // suse (Okta): require the `email` claim. Okta commonly OMITS email_verified,
  // so we reject only an explicit false rather than demanding true. We do NOT
